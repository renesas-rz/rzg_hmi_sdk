...
static void Image_Gallery_btn_cb(lv_event_t* e)
{
    lv_obj_send_event(Image_Gallery_cont, LV_EVENT_CLICKED, NULL);
}

static void Home_Automation_btn_cb(lv_event_t* e)
{
    lv_obj_send_event(Home_Automation_cont, LV_EVENT_CLICKED, NULL);
}

static void Weather_History_btn_cb(lv_event_t* e)
{
    lv_obj_send_event(Weather_History_cont, LV_EVENT_CLICKED, NULL);
}
    ...
    static void prev_click_event_cb(lv_event_t * e)
    {
        ...
        lv_image_set_src(music_state_btn_icon, &music_btn_pause_icon);
        lv_label_set_text_fmt(music_num_text, "Playing %d of 3", music_num + 1);
        lv_image_set_src(music_image, music_list[music_num].img);
    ...
    static void music_loop_btn_click_cb(lv_event_t * e)
    {
        ...
        if(loop_state == 0)
        {
            lv_image_set_src(loop_btn_icon, &music_btn_loop_on_icon);
            loop_state = 1;
        }
        /* loop_enable */
    ...
    static void music_volume_event_cb(lv_event_t* e)
    {
        ...
        g_object_set(G_OBJECT(volume), "volume", volume_value, NULL);
        lv_label_set_text_fmt(volume_value_label, "%d", (int)volume_slider_value * 10);
    }
    ...
    void cb_print_position(void)
    {
        static gint64 cached_music_len = -1;
        guint basesec = 0;
        guint hours = 0;
        guint minites = 0;
        guint seconds = 0;

        if (state == INIT || state == WAIT_LENGTH)
        {
        /* The length is only retrieved when the music_num changes or when the song information is in the initial state. */
            if(gst_element_query_duration(pipeline, GST_FORMAT_TIME, &music_len))
            {
                cached_music_len = music_len;
                state = WAIT_POSITION;
            }
        }

        if (cached_music_len <= 0)
        {
            /* Error because length cannot be obtained */
            return;
        }

        if (state == PLAYING || state == WAIT_POSITION)
        {
            if(gst_element_query_position (pipeline, GST_FORMAT_TIME, &music_pos))
            {
                if(state == WAIT_POSITION)
                {
                    ...
                    lv_label_set_text(music_name, music_list[music_num].name);
                    lv_label_set_text(music_artist, music_list[music_num].artist);
                    basesec = (guint)(cached_music_len / GST_SECOND);
                    hours =   basesec / 3600;
                    minites = (basesec % 3600) / 60;
                    seconds = basesec % 60;
                    lv_label_set_text_fmt(end_time_text, "%02u:%02u:%02u\n", hours, minites, seconds);
                    state = PLAYING;
                }
                /* Calculate and display the obtained values */
                music_time =  ((double)music_pos / (double)cached_music_len)*100 +0.1;
                lv_slider_set_value(time_slider, (int)music_time, 0);
                basesec = (guint)(music_pos / GST_SECOND);
                hours =   basesec / 3600;
                minites = (basesec % 3600) / 60;
                seconds = basesec % 60;
                lv_label_set_text_fmt(time_text, "%02u:%02u:%02u\n", hours, minites, seconds);
            }

            if((loop_state == 1 && music_time >= 100) && state == PLAYING)
            {
                music_time = 0;
