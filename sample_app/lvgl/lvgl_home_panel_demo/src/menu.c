...
uint16_t img_ratio_calc(lv_obj_t* img,uint16_t scr_width, uint16_t scr_height);
...
void create_menu()
{
    ...
    lv_obj_t* Dashboard_tag_icon = lv_img_create(Dashboard_cont);
    lv_img_set_src(Dashboard_tag_icon, &Dashboard_icon);
    ...
    lv_event_send(Dashboard_cont, LV_EVENT_CLICKED, NULL);

    //Image_Gallery_menu
    Image_Gallery = lv_menu_page_create(menu, NULL);
    Image_Gallery_section = lv_menu_section_create(Image_Gallery);
    ...
    lv_obj_t* Image_Gallery_tag_icon = lv_img_create(Image_Gallery_cont);
    lv_img_set_src(Image_Gallery_tag_icon, &Image_Gallery_icon);
    ...
    Home_Automation_section = lv_menu_section_create(Home_Automation);
    ...
    lv_obj_t* Home_Automation_tag_icon = lv_img_create(Home_Automation_cont);
    lv_img_set_src(Home_Automation_tag_icon, &Home_Automation_icon);
    ...
    Weather_History_section = lv_menu_section_create(Weather_History);
    ...
    lv_obj_t* Weather_History_tag_icon = lv_img_create(Weather_History_cont);
    lv_img_set_src(Weather_History_tag_icon, &Weather_History_icon);
    ...
    lv_label_set_text(Version_text, "Ver.1.00");
...
static void back_event_handler(lv_event_t* e)
{
    if (sidebar_state == 0) //sidebar_Open
    {
        lv_menu_t* menu_t = (lv_menu_t*)menu;
        lv_obj_set_width(menu_t->sidebar, 57);
        sidebar_state = 1;
    }
    else if (sidebar_state == 1) //sidebar_Close
    {
        lv_menu_t* menu_t = (lv_menu_t*)menu;
        lv_obj_set_width(menu_t->sidebar, 320);
        sidebar_state = 0;
    }
}
...
uint16_t img_ratio_calc(lv_obj_t* img,uint16_t scr_width, uint16_t scr_height)
{
    ...
    uint16_t img_width;
    uint16_t img_height;
    uint16_t img_obj_width = lv_obj_get_width(img);
    uint16_t img_obj_height = lv_obj_get_height(img);
